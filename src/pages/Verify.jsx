...
const Verify = () => {
  ...
  const getCameraErrorMessage = (err) => {
    if (!window.isSecureContext) {
      return 'Camera requires a secure page. Open this app on localhost or HTTPS.';
    }

    if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
      return 'Camera is not supported in this browser. Try the latest Chrome or Edge.';
    }
    ...
    switch (err?.name) {
      ...
      default:
        return 'Unable to start camera. Please retry and check browser permissions.';
    }
  };
  ...
  const releaseStream = useCallback((stream) => {
    ...
    stream.getTracks().forEach((track) => {
      try {
        ...
        // Ignore stop failures from already-ended tracks.
      }
    });
  }, []);
  ...
  const attachStreamToVideo = useCallback(async (stream) => {
    ...
    try {
      ...
      // Some browsers need metadata loaded before play can succeed.
    }
    ...
    await new Promise((resolve) => {
      const onLoaded = async () => {
        ...
        try {
          ...
          setError('Camera permission granted, but preview could not start. Please click Start Camera again or close other apps using camera.');
        }
        resolve();
      };
  ...
  useEffect(() => {
    if (user?.aadhaarNumber) {
      setAadhaarNumber(formatAadhaar(user.aadhaarNumber));
    }
  }, [user]);
  ...
  const handleAadhaarVerify = async () => {
    ...
    if (result) {
      const mapped = {
        ...
        dob: result.dob || user?.dateOfBirth || '',
        state: result.state || user?.state || '',
        constituency: result.constituency || user?.constituency || ''
      };
      setAadhaarData(mapped);
      setVerificationStatus('verified');
      setTimeout(() => setStep(2), 1000);
    } else {
      setVerificationStatus('failed');
      setError('Aadhaar not found in records. Please use the Aadhaar used during registration.');
    }
  };
  ...
  const startCamera = async () => {
    ...
    if (!window.isSecureContext) {
      setError('Camera requires a secure page. Use http://localhost:5173 or HTTPS.');
      setStatusMsg('');
      return;
    }

    if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
      setError('This browser does not support camera access. Use the latest Chrome or Edge.');
      setStatusMsg('');
      return;
    }
  ...
  const stopCamera = useCallback(() => {
    ...
    releaseStream(pendingStreamRef.current);
  ...
  const capturePhoto = () => {
    ...
    if (!video.videoWidth || !video.videoHeight) {
      setError('Camera is not ready yet. Wait a moment and try Capture Photo again.');
      return;
    }
  ...
  const proceedToVoting = () => navigate('/citizen/vote');
        ...
        {step === 1 && (
          <div className="verify-card">
            ...
            <div className="card-content">
              ...
              {verificationStatus !== 'verified' && !user?.aadhaarNumber && (
